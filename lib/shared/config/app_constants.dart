...
///
/// Grouping constants here ensures that a wire-protocol string
/// (collection name, SharedPreferences key, duration, etc.) is changed in one
/// place. See `docs/architecture/ADR-001-firestore-contracts.md`.
library;

// ─── Firestore collection names ─────────────────────────────────────────────

/// Top-level constants for every Firestore collection used by the app.
/// Prevents typo-driven bugs. **Do not rename values** without a multi-client migration.
abstract final class FirestoreCollections {
  ...
  /// Per-user alert inbox subcollection under `users/{uid}/`.
  static const String alertInbox = 'alert_inbox';

  /// Soft inbox subcollection under `users/{uid}/` (messages + membership events).
  static const String communityMessages = 'community_messages';
}
...
abstract final class AlertFields {
  ...
  /// Legacy companion flags written with embedded images (optional on reads).
  static const String hasImages = 'hasImages';
  static const String imageCount = 'imageCount';
