...
import 'package:guardian/features/inbox/presentation/widgets/community_message_detail_dialog.dart';
...
class _NotificationsViewState extends State<NotificationsView> {
  ...
  List<CommunityInboxItem> _scopeToMemberships(
    List<CommunityInboxItem> items,
    Set<String> memberCommunityIds,
  ) {
    return items.where((m) {
      if (m.isMembershipEvent) return true;
      final id = m.communityId;
      if (id != null && memberCommunityIds.contains(id)) return true;
      return m.communityIds.any(memberCommunityIds.contains);
    }).toList();
  }

  void _maybeInvalidateAlertsOnKick(List<CommunityInboxItem> items) {
    if (_didInvalidateOnKick) return;
    final hasKick = items.any(
      (m) =>
          m.kind == CommunityInboxFields.kindMemberRemoved && m.isUnread,
    );
    if (!hasKick) return;
  ...
  List<CommunityInboxItem> _applyFilters(List<CommunityInboxItem> items) {
    var list = List<CommunityInboxItem>.from(items);

    if (_communityFilter != null) {
      list = list.where((m) {
        final id = m.communityId;
        return id == _communityFilter ||
            m.communityIds.contains(_communityFilter);
      }).toList();
    }
  ...
  List<MapEntry<String, String>> _communityOptions(
    ...
    for (final m in items) {
      final id = m.communityId;
      final name = m.communityName?.trim();
  ...
  Future<void> _openMessage(String uid, CommunityInboxItem item) async {
    if (item.isUnread) {
      await _messageService.markRead(uid, item.id);
    }
    if (!mounted) return;
    await CommunityMessageDetailDialog.show(
      context,
        ...
        child: uid == null
            ...
            : StreamBuilder<List<CommunityModel>>(
                ...
                builder: (context, membershipSnap) {
                  ...
                  return StreamBuilder<List<CommunityInboxItem>>(
...
class _InboxRow extends StatelessWidget {
  ...
  final CommunityInboxItem message;
  final VoidCallback onOpen;

  @override
  Widget build(BuildContext context) {
    return message.isCommunityMessage
...
class _MessageRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final community = message.communityName?.trim();
