...
class _CommunityReportsViewState extends State<CommunityReportsView>
  ...
  List<MemberReportListItem> _reports = [];
  ...
  Future<void> _dismissReport(MemberReportListItem report) async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text(
          AppLocalizations.of(context)!.dismissQuestion(report.reportedUserName),
          style: TextStyle(
            fontSize: 15,
    ...
    _showLoadingOverlay();
    final success = await _communityService.dismissReport(report.reportId);
    if (!mounted) return;
  ...
  Future<void> _removeReportedUser(MemberReportListItem report) async {
    final l10n = AppLocalizations.of(context)!;
    final confirmed = await showDialog<bool>(
      context: context,
      builder: (context) => AlertDialog(
        ...
        content: Text(
          AppLocalizations.of(context)!.expelFromReportsConfirmation(report.reportedUserName),
          style: TextStyle(
            fontSize: 15,
            color: Colors.grey[700],
            height: 1.4,
    ...
    if (removeSuccess) {
      await _communityService.dismissReport(report.reportId);
    }
    ...
    if (removeSuccess) {
      _showSnackBar(
        '${report.reportedUserName} ha sido expulsado',
        isSuccess: true,
      );
      _loadReports();
    } else {
  ...
  Widget _buildReportCard(MemberReportListItem report) {
    final reportedName = report.reportedUserName;
    final reportedBy = report.reportedByUserName;
